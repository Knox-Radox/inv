...
/**
 * The garland's leaves, grouped into five swaying bands.
 *
 * The anchor is the cord at the band's own midpoint, so each section turns
 * about where it is tied rather than about the middle of the frame.
 */
const BAND_COUNT = 5;
const BANDS = Array.from({ length: BAND_COUNT }, (_, bi) => {
  const leaves = TH.leaves
    .map((leaf, i) => ({ leaf, i }))
    .filter(({ leaf }) => Math.min(BAND_COUNT - 1, Math.floor(leaf.t * BAND_COUNT)) === bi);
  const mid = leaves[Math.floor(leaves.length / 2)]?.leaf;
  return {
    leaves,
    ax: mid?.x ?? 0,
    ay: mid?.y ?? 0,
    // Primes-ish, so no two bands come back into step within a sitting.
    sway: 5.3 + bi * 1.37,
    phase: bi * 1.9,
  };
});
...
function Thoranam({ className }: { className?: string }) {
  return (
    ...
    >
      ...
      {/*
       * Swayed in five bands, not leaf by leaf.
       *
       * Every leaf had its own rotation, and every rotated SVG group is a
       * main-thread repaint — Chrome will not promote an SVG `<g>` to its own
       * composited layer the way it will an HTML box. Twenty-six of them here
       * and fifty-four on the malai put frame pacing at p95 36.5 ms on a 4x
       * CPU and 74.5 ms on a 6x one, scrolling.
       *
       * It is also more truthful. A thoranam strung across a doorway does not
       * flutter leaf by leaf; it swings in sections, because the cord carries
       * the motion. Five bands, each on its own period and phase.
       */}
      {BANDS.map((band, bi) => (
        <g
          key={`band${bi}`}
          ...
          style={
            {
              "--anchor": `${band.ax}px ${band.ay}px`,
              "--sway": `${band.sway}s`,
              "--phase": `${-band.phase}s`,
            } as React.CSSProperties
          }
        >
          {band.leaves.map(({ leaf, i }) => {
            // Each leaf still takes its cue from where the cord has reached, so
            // the string hangs itself rather than arriving all at once. The
            // same mechanic as the jasmine spray on the card, which is the one
            // piece of motion on this page the client asked for more of.
