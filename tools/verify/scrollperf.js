/*
 * Frame pacing while a guest scrolls the field, which is where all the
 * ornament and the map plate live.
 *
 * perf.js measures the opening; this measures everything after it. Revision 7
 * needed it because the plate's road reveal moved from a clipPath — which was
 * not animating at all, a mask being the only one of the two that honours a
 * dash — to a mask, and a mask costs more per frame than a clip that did
 * nothing.
 *
 *   node tools/verify/scrollperf.js "$URL"
 */
...
(async () => {
  ...
  for (const rate of [1, 4, 6]) {
    ...
    // A steady walk down the whole field, at about a thumb's pace.
    for (let i = 0; i < 60; i++) { await p.mouse.wheel(0, 260); await p.waitForTimeout(55); }
